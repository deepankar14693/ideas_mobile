...
export const getMobileDashboard = (groupId, scrType, valueType, recommendation, decision) => {
    return [(dispatch, getState) => {
        dispatch(showNotification('Loading...'));
    },
    (dispatch, getState) => {
        dispatch(getMobileDashboardData(groupId, scrType, valueType, recommendation, decision))
    }]
};

const getMobileDashboardData = (groupId, scrType, valueType, recommendation, decision) => {
    let params = { callTime: new Date(), scrType: scrType };
    let url = '';
    const currentTimeStamp = getMomentTimeStamp(new Date());
    ...
    let userId = memoryStore.getItem("userId");

    scrType = scrType ? scrType : 1;
    valueType = valueType ? valueType : 1;
    recommendation = recommendation ? recommendation : 0;
    decision = decision ? decision : 0;

    if (groupId === '00000000-0000-0000-0000-000000000000') {
        url = 'Mobile/GetDashboardData/' + projectId + '-' + scrType + '-' + valueType + '-' + recommendation + '-' + decision + '-' + timeStamp + '-' + userId + '-' + permissionTimeStamp;
    } else {
        let ideasTimeStamp = currentTimeStamp; /* (timeStamps.Groups && timeStamps.Groups[groupId.toLowerCase()]) ? (getMomentTimeStamp(timeStamps.Groups[groupId.toLowerCase()].GroupIdeas)) : */
        url = 'Mobile/GetDashboardData/' + groupId + '-' + scrType + '-' + valueType + '-' + recommendation + '-' + decision + '-' + ideasTimeStamp + '-' + userId + '-' + permissionTimeStamp;
    }

    const request = getAxios(url, { params: params });
    return [{
        type: actionTypes.GET_MOBILE_DASHBOARD_DATA,
        payload: request
    }, (dispatch, getState) => {
...
export const getDashboardCheckList = (groupId) => {
    return [(dispatch, getState) => {
        dispatch(showNotification('Loading...'));
    },
    (dispatch, getState) => {
        dispatch(getDashboardCheckListData(groupId))
    }]
};

const getDashboardCheckListData = (groupId) => {
    let params = { callTime: new Date() };
    let url = '';
    const currentTimeStamp = getMomentTimeStamp(new Date());
    const projectId = AppConfig.env('projectId');
    ...
    if (groupId === '00000000-0000-0000-0000-000000000000') {
        url = 'Mobile/GetDashboardCheckList/' + projectId + '-' + timeStamp + '-' + userId + '-' + permissionTimeStamp;
    } else {
        let ideasTimeStamp = currentTimeStamp; /* (timeStamps.Groups && timeStamps.Groups[groupId.toLowerCase()]) ? (getMomentTimeStamp(timeStamps.Groups[groupId.toLowerCase()].GroupIdeas)) : */
        url = 'Mobile/GetDashboardCheckList/' + groupId + '-' + ideasTimeStamp + '-' + userId + '-' + permissionTimeStamp;
    }

    const request = getAxios(url, { params: params });
    return [{
        type: actionTypes.GET_DASHBOARD_CHECKLIST,
        payload: request
    }, (dispatch, getState) => {
        dispatch(hideNotification())
    }]
