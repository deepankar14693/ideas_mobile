    ...
    switch (view) {
        ...
        case 15:
            switch (action.type) {
                case 'IDEA_SELECT':
                    return update(state, {
                        ideasSentButRejectedView: { selectedIdea: { $set: action.payload } },
                    });
                case 'IDEA_COUNT':
                case 'IDEAGROUP_COUNT':
                    return update(state, {
                        ideasSentButRejectedView: { ideaCount: { $set: action.payload } },
                    });

                case 'IS_SHOW_INACTIVE_IDEA':
                    setLocalStorageKey('isShowInactiveIdeaIdeasSentButRejectedView', action.payload);
                    return update(state, {
                        ideasSentButRejectedView: { isShowInactiveIdea: { $set: action.payload } },
                    });

                case 'IS_SHOW_HIGHLIGHTED_IDEA':
                    setLocalStorageKey('isShowHighlightedIdeaIdeasSentButRejectedView', action.payload);
                    return update(state, {
                        ideasSentButRejectedView: { isShowHighlightedIdea: { $set: action.payload } },
                        ideasSentButRejectedView: { ideasOnCurrentPage: { $set: action.ideasOnCurrentPage } },
                    });

                case 'IS_SHOW_IDEA_RISK_DISTRIBUTION':
                    setLocalStorageKey('isShowRiskDistributionIdeasSentButRejectedView', action.payload);
                    return update(state, {
                        ideasSentButRejectedView: { isShowRiskDistribution: { $set: action.payload } },
                    });
                case 'PAGE_CHANGE':
                case 'PAGE_CHANGE_IDEAGROUPS':
                case 'RESET_PAGING':
                case 'RESET_PAGING_IDEAGROUPS':
                    if (action.type === 'RESET_PAGING' || action.type === 'RESET_PAGING_IDEAGROUPS') {
                        return update(state, {
                            ideasSentButRejectedView: { currentPage: { $set: 1 } },
                        });
                    }
                    if (action.type === 'PAGE_CHANGE' || action.type === 'PAGE_CHANGE_IDEAGROUPS') {
                        return update(state, {
                            ideasSentButRejectedView: { currentPage: { $set: action.payload } },
                        });
                    }
                case 'PERPAGE_IDEACOUNT_CHANGE':
                    utils.setLocalStorageKey('recordsPerPageideasSentButRejectedView1', action.payload);
                    return update(state, {
                        ideasSentButRejectedView: { perPageIdeaCount: { $set: action.payload } },
                        ideasSentButRejectedView: { currentPage: { $set: 1 } },
                    });
                case 'SET_IDEA_COUNTONPAGE':
                    return update(state, {
                        ideasSentButRejectedView: { ideasOnCurrentPage: { $set: action.payload } },
                    });
                case 'CHECK_ALL_IDEAS_FROM_ONE_PAGE':
                    return update(state, {
                        ideasSentButRejectedView: { isSelected: { $set: action.payload } },
                        ideasSentButRejectedView: {
                            checkedIdeas: {
                                $set: (
                                    action.payload ? state.ideasSentButRejectedView.checkedIdeas + state.ideasSentButRejectedView.ideasOnCurrentPage :
                                        state.ideasSentButRejectedView.checkedIdeas - state.ideasSentButRejectedView.ideasOnCurrentPage
                                )
                            }
                        },
                        ideasSentButRejectedView: {
                            checkedIdeasOnCurrentPage: {
                                $set: (
                                    action.payload ? state.ideasSentButRejectedView.ideasOnCurrentPage : 0
                                )
                            }
                        },
                    });
                ...
                case 'UNCHECK_ALL_IDEAS':
                    return update(state, {
                        ideasSentButRejectedView: { isSelected: { $set: action.payload } },
                        ideasSentButRejectedView: { checkedIdeas: { $set: 0 } },
                        ideasSentButRejectedView: { checkedIdeasOnCurrentPage: { $set: 0 } },
                    });
                ...
                case 'CHECK_IDEA':
                    return update(state, {
                        ideasSentButRejectedView: {
                            checkedIdeas: {
                                $set: (
                                    action.checked ? state.ideasSentButRejectedView.checkedIdeas + 1 : state.ideasSentButRejectedView.checkedIdeas - 1
                                )
                            }
                        },
                        ideasSentButRejectedView: {
                            checkedIdeasOnCurrentPage: {
                                $set: (
                                    action.checked ? state.ideasSentButRejectedView.checkedIdeasOnCurrentPage + 1 : state.ideasSentButRejectedView.checkedIdeasOnCurrentPage - 1
                                )
                            }
                        },
                        ideasSentButRejectedView: {
                            isSelected: {
                                $set:
                                    ((state.ideasSentButRejectedView.ideasOnCurrentPage === state.ideasSentButRejectedView.checkedIdeasOnCurrentPage) ? true : false)
                            }
                        },
                    });
                case 'SET_IDEA_STATES_ON_SELECTED_PAGE':
                    ...
                    const filteredIdeas = _.filter(ideaState, { 'IsCurrentlyShowing': true });
                    const totalFilteredIdeas = filteredIdeas.length;
                    const totalCheckedIdeas = filteredIdeas.filter(function (el) { return el.isIdeaCheckedIdeasSentButRejected }).length;
                    return update(state, {
                        ideasSentButRejectedView: { ideasOnCurrentPage: { $set: totalFilteredIdeas } },
                        ideasSentButRejectedView: { checkedIdeasOnCurrentPage: { $set: totalCheckedIdeas } },
                        ideasSentButRejectedView: {
                            isSelected: {
                                $set:
                                    ((totalFilteredIdeas === totalCheckedIdeas) && totalFilteredIdeas !== 0 && totalCheckedIdeas !== 0) ? true : false
...
const filterReducer = (state = [], action, entireState) => {
    switch (action.type) {
        ...
        case 'SET_SEARCHTEXT_IMPLEMENTATION':
            return update(state, {
                searchTextForIdeaAndImplementation: { $set: action.payload },
            });
        default:
            return state;
    }
